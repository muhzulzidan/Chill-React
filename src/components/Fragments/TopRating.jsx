...
import MovieHoverCardV2 from "../Elements/MovieCard/MovieHoverCardV2";
...
const TopRating = () => {
  ...
  const settings = {
    dots: false,
    infinite: movies && movies.length >= 5,
    speed: 700,
    slidesToShow: 5, // Default untuk laptop/PC
    slidesToScroll: 3,
    arrows: false,
    responsive: [
      {
      ...
      {
        breakpoint: 640, // HP kecil
        settings: { slidesToShow: 2, slidesToScroll: 1 },
      },
      {
        breakpoint: 480, // HP sangat kecil
        settings: { slidesToShow: 1, slidesToScroll: 1, centerMode: true, centerPadding: "40px" },
      },
    ],
  };

  if (loading) return <div className="h-[200px] flex justify-center items-center"><p>Loading Top Movies...</p></div>;
  if (error) return <div className="h-[200px] flex justify-center items-center"><p>Error: {error.message}</p></div>;
  if (!movies || movies.length === 0) return <div className="h-[200px] flex justify-center items-center"><p>No Top Movies.</p></div>;
